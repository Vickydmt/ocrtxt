...
/** Fallback to Tesseract.js when Google Vision fails */
async function tesseractFallback(file: File): Promise<string> {
  ...
  try {
    await worker.loadLanguage("eng+fra+spa+deu"); // Common historical doc languages
    await worker.initialize("eng+fra+spa+deu");
